...
public class MapChangeProcessorTests
{
    private readonly Mock<ILogger<MapChangeProcessor>> _logger = new();
    ...
    public MapChangeProcessorTests()
    {
    ...
    public async Task ValidEvent_CreatesMapChangeEvent()
    {
        ...
        await _sut.ProcessMapChange(message, _functionContext.Object);

        _eventsApi.Verify(x => x.CreateGameServerEvent(It.Is<CreateGameServerEventDto>(dto =>
            dto.GameServerId == TestServerId &&
            dto.EventType == "MapChange" &&
            dto.EventData.Contains("mp_crash")), It.IsAny<CancellationToken>()), Times.Once);
    }
    ...
    [Fact]
    public async Task EmptyMapName_LogsWarningAndReturns()
    {
        var evt = CreateValidEvent(mapName: "");
        var message = CreateMessage(evt);
